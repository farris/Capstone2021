...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # if args.aug_plus: # DONE: swap to torchio transforms
    ...
    augmentation = torchio.Compose([
        ...
        torchio.RandomGamma(),
        torchio.RandomAffine(
            scales=(1.2, 1.5) # TODO: insert random cropping code
        )
    ])
    # else:
    ...
    labels = labels[labels['torch_present']] 
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, images in enumerate(train_loader): # DONE: test this with our dataloader
        # measure data loading time
        data_time.update(time.time() - end)

        # DONE: discard iop/icp labels
        images = images[0]
        if args.gpu is not None:
